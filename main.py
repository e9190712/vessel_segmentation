...
parser.add_argument("--mode", default='train', type=str, help="k_fold_train ,train, test_single_img, test_All_PR, make_npz_dataset")#模式選擇(test_single_img-->single image eval，test_All_PR-->muti image eval)
...
parser.add_argument("--epoch", default=1, type=int, help='number of passes of the entire training dataset the machine learning algorithm has completed')
parser.add_argument("--size", default=512, type=int, help='training img size')
parser.add_argument("--batch_size", default=8, type=int, help='number of samples that will be propagated through the network')
...
parser.add_argument("--load_npz_path", default='/media/NAS/mike2/npz_dataset/dataset_500_1/set.npz', type=str, help='CAG set')#load訓練圖像，已經將png轉成npz，這樣load時會比較快
