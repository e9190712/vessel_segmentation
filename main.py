...
parser.add_argument("--epoch", default=200, type=int, help='number of passes of the entire training dataset the machine learning algorithm has completed')
parser.add_argument("--size", default=512, type=int, help='training img size')
parser.add_argument("--batch_size", default=2, type=int, help='number of samples that will be propagated through the network')
...
parser.add_argument("--load_npz_path", default=r'C:\Users\Chu\Desktop\work_fold\dataset_5_fold_NoAug(gray)_new\set.npz', type=str, help='CAG set')#load訓練圖像，已經將png轉成npz，這樣load時會比較快
...
parser.add_argument("--dataset_path", default=r'C:\Users\Chu\Desktop\work_fold\CAG_segmentation\500_5fold', type=str)
parser.add_argument("--save_npz_path", default=r'C:\Users\Chu\Desktop\work_fold\dataset_5_fold_NoAug(gray)_new', type=str, help='CAG set')
parser.add_argument("--open_k_fold", default=True, type=bool, help='save k fold set')
...
################# optimizer parameter #################
parser.add_argument("--init_lr", default=3e-5, type=float, help='tuning parameter in an optimization algorithm that determines the step size at each iteration while moving toward a minimum of a loss function')
...
if args.mode == 'k_fold_train':
    ...
    train_labels = [args.dataset_path+'/'+name+'/new_labels' for name in training_data if 'train' in name]
    val_imgs = [args.dataset_path+'/'+name+'/imgs' for name in training_data if 'val' in name]
    val_labels = [args.dataset_path+'/'+name+'/new_labels' for name in training_data if 'val' in name]
    save_npz(train_imgs, train_labels, val_imgs, val_labels, args.save_npz_path, 'set.npz', label_div_255=False, k_fold=args.open_k_fold)