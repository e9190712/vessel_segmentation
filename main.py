...
parser.add_argument("--GPU_num", default='3', type=str, help="selected GPU cores")
parser.add_argument("--mode", default='test_single_img', type=str, help="k_fold_train ,train, test_single_img, test_All_PR, make_npz_dataset")#模式選擇(test_single_img-->single image eval，test_All_PR-->muti image eval)
...
parser.add_argument("--k_fold", default=None, type=int, help='fold num, None is not k_fold') # set 1, 2,.... and None
parser.add_argument("--epoch", default=1, type=float, help='number of passes of the entire training dataset the machine learning algorithm has completed')
parser.add_argument("--size", default=512, type=float, help='training img size')
parser.add_argument("--batch_size", default=8, type=float, help='number of samples that will be propagated through the network')
...
parser.add_argument("--backbone", default='vgg16', type=str, help="inceptionresnetv2, densenet121, vgg16, densenet201, senet154, resnext50, resnext101")#選擇backbone
parser.add_argument("--load_npz_path", default='/media/NAS/mike2/npz_dataset/dataset_C/set.npz', type=str, help='CAG set')#load訓練圖像，已經將png轉成npz，這樣load時會比較快
parser.add_argument("--save_model", default='/media/NAS/mike2/higher_score_model_and_result/Unet_with_vgg_test_model', type=str, help='path of save model')
parser.add_argument("--load_model", default='/media/NAS/mike2/higher_score_model_and_result/Unet_with_vgg_test_model', type=str, help='path of load model')
